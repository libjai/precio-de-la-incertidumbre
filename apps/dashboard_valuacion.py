...
vol implícita con sonrisa a partir de cotizaciones. Cifras por acción y por contrato de
100 acciones (convención MexDer).
...
def render() -> None:
    ...
    with st.sidebar:
        ...
        S = st.slider("Spot S (MXN por acción)", 1.0, 200.0, 12.50, 0.5, key="v_S")
        K = st.slider("Strike K (MXN)", 1.0, 200.0, 13.00, 0.5, key="v_K")
        sig = st.slider("Volatilidad σ (% anual)", 5.0, 100.0, 28.0, 1.0, key="v_sig") / 100
        r = st.slider("Tasa r (% anual, continua)", 0.0, 20.0, 9.5, 0.25, key="v_r",
                      help="Referencia mexicana: TIIE / Cetes.") / 100
        T = st.slider("Tiempo al vencimiento T (años)", 0.05, 3.0, 0.5, 0.05, key="v_T",
                      help="0.5 = seis meses (≈ 182 días).")
        st.caption(f"T = {gf.etiqueta_plazo(T)}")
        ...
        m = st.select_slider("Trayectorias LSM", [20_000, 60_000, 120_000], value=60_000, key="v_m",
                             help="Más trayectorias = menos error de simulación, más tiempo de cálculo.")
    ...
    c1.metric("Europea (BSM)", f"{euro:.4f}", border=True,
              help="Solo se puede ejercer al vencimiento. Fórmula cerrada.")
    c2.metric("Americana (CRR, N = 1200)", f"{amer:.4f}", border=True,
              help="Se puede ejercer en cualquier momento, como las opciones sobre acciones de MexDer. "
                   "Árbol binomial con 1200 pasos.")
    c3.metric("Americana (LSM)", f"{lsm:.4f}", f"± {se:.4f} error estándar",
              delta_color="off", border=True,
              help="Monte Carlo con regresión (Longstaff-Schwartz). El ± es la incertidumbre estadística.")
    c4.metric("Premio por ejercicio temprano", f"{amer - euro:.4f}",
              help="Americana (CRR) − Europea (BSM): cuánto vale el derecho a ejercer antes.", border=True)

    st.markdown(f"**Por contrato ({ui.LOTE} acciones) y por posición ({contratos} contrato"
                f"{'s' if contratos != 1 else ''} = {n:,} acciones), en {ui.MONEDA}**")
    d1, d2, d3, d4 = st.columns(4)
    d1.metric("Europea (BSM) · contrato", f"{euro * ui.LOTE:,.2f}", f"posición {euro * n:,.2f}",
              delta_color="off", border=True)
    d2.metric("Americana (CRR) · contrato", f"{amer * ui.LOTE:,.2f}", f"posición {amer * n:,.2f}",
              delta_color="off", border=True)
    d3.metric("Americana (LSM) · contrato", f"{lsm * ui.LOTE:,.2f}",
              f"posición {lsm * n:,.2f} ± {se * n:,.2f}", delta_color="off", border=True)
    d4.metric("Premio ejercicio temprano · contrato", f"{(amer - euro) * ui.LOTE:,.2f}",
              f"posición {(amer - euro) * n:,.2f}", delta_color="off", border=True)
    st.caption(f"Convención MexDer: cada contrato de opción sobre acciones ampara {ui.LOTE} acciones "
               "y no se operan fracciones de contrato. La prima se cotiza por acción; lo que pagas "
               f"por un contrato es prima × {ui.LOTE}.")
    ...
    with tab_st:
        ...
        if vista == "precio":
            fig = gf.fig_mapa(modo, amer_g, Ss, Ts, "Spot S (MXN)", "Tiempo al vencimiento T (años)",
                              f"Precio del {tipo} americano (binomial CRR, N = 200)", (S, T),
                              "Precio (MXN por acción)", color)
            ui.grafico_con_nota(
                fig,
                f"**Qué ves:** el precio del {tipo} americano en una malla de 9 × 9 "
                f"combinaciones de spot (±30 %) y tiempo al vencimiento (de 0.25T a 1.75T, es "
                f"decir, de {gf.etiqueta_plazo(T * 0.25)} a {gf.etiqueta_plazo(T * 1.75)}), con "
                f"σ = {sig * 100:.0f} % y r = {r * 100:.2f} % fijos.\n\n"
                ...
                "queda.\n\n"
                f"**Unidades:** pesos por acción (× {ui.LOTE} = contrato); T en años.")
            _tabla(amer_g, Ts, Ss, "T = {:.2f}", "S = {:.2f}")
        else:
            fig = gf.fig_mapa(modo, amer_g - euro_g, Ss, Ts, "Spot S (MXN)", "Tiempo al vencimiento T (años)",
                              f"Premio por ejercicio temprano del {tipo} (americana − europea)",
                              (S, T), "Premio (MXN por acción)", color)
            ui.grafico_con_nota(
                fig,
                ...
                f"**Unidades:** pesos por acción (× {ui.LOTE} = contrato).")
            _tabla(amer_g - euro_g, Ts, Ss, "T = {:.2f}", "S = {:.2f}")

    with tab_frontera:
        res = ui.frontera_ejercicio(S, K, r, sig, T, tipo)
        if res is None:
            st.info("Un call sobre una acción que no paga dividendos nunca se ejerce antes del "
                    "vencimiento: siempre vale más venderlo que ejercerlo (Cap. 8). Cambia a **put** "
            ...
            ui.grafico_con_nota(
                ...
                "del árbol CRR (N = 600): en cada fecha, el spot más alto en el que conviene "
                "ejercer en vez de esperar.\n\n"
                "**Cómo leerlo:** eje x = tiempo transcurrido desde hoy (en años; el extremo "
                f"derecho es el vencimiento, T = {gf.etiqueta_plazo(T)}), eje y = spot. Por debajo "
                "de la curva (zona sombreada) se ejerce; por encima se espera. La frontera sube "
                "hacia K al acercarse el vencimiento: cuanto menos tiempo queda, menos vale "
                "esperar. Los pequeños escalones son la malla del árbol. Es exactamente la regla "
                "de parada que Longstaff-Schwartz aprende por regresión (Cap. 9). Tu spot de hoy "
                "está marcado en t = 0.\n\n"
                "**Unidades:** spot en pesos por acción; tiempo en años.")
    ...
    with tab_conv:
        ...
        ui.nota(
            ...
            "banda de ±1.96 errores estándar (el intervalo del 95 %); la línea gris es la "
            "americana CRR con N = 1200.\n\n"
            "**Cómo leerlo:** ambos ejes x son logarítmicos (cada marca multiplica por 10). El "
            "zigzag par-impar del binomial es normal (Cap. 4). La banda de LSM se estrecha con "
            "√m: cuadruplicar las trayectorias reduce el error a la mitad (Cap. 7). Si la línea "
            "gris cae fuera de la banda con muchas trayectorias, sospecha del número de pasos o "
            "de las funciones base (Cap. 9).\n\n"
            "**Unidades:** pesos por acción.")
    ...
    with tab_sr:
        ...
        fig = gf.fig_mapa(modo, grid, sigs_pct, rs_pct, "Volatilidad σ (%)", "Tasa r (%)",
                          ...
                          (sig * 100, r * 100), "Precio (MXN por acción)", color)
        ...
        ui.grafico_con_nota(
            ...
            "(±15 puntos porcentuales) y tasa (±4 puntos porcentuales) alrededor de tu "
            "configuración, con S, K y T fijos.\n\n"
    ...
    with tab_iv:
        ...
        with col_son:
            ...
            cotiz = st.data_editor(st.session_state["v_cotiz_base"], num_rows="dynamic", hide_index=True,
                                   ...
                                   column_config={"Strike": st.column_config.NumberColumn(format="%.2f"),
                                                  "Precio de mercado": st.column_config.NumberColumn(
                                                      "Precio de mercado (MXN/acción)", format="%.4f")})
            cotiz = cotiz.dropna()
            ...
            ui.nota(
                ...
                f"observado del {tipo}, por strike, con S = {S:g}, r = {r * 100:.2f} % y T = "
                f"{gf.etiqueta_plazo(T)} fijos. La línea gris es la σ plana de tu modelo. Las "
                "cotizaciones iniciales son sintéticas con skew de renta variable (más vol en "
                "strikes bajos): sustitúyelas por precios de mercado y verás la sonrisa real.\n\n"
                ...
                "**Unidades:** vol en % anual; precios en pesos por acción.")
        with col_map:
            st.markdown("**Vol implícita en strike × precio** (la inversión completa)")
            Ks, Ps_iv, iv_g = ui.mapa_vol_implicita(S, K, r, T, tipo)
            fig = gf.fig_mapa(modo, iv_g, Ks, Ps_iv, "Strike K (MXN)", f"Precio del {tipo} (MXN/acción)",
        ...
        with st.expander("Vista no estándar: tasa implícita en strike × precio (σ fija)"):
            ...
            fig = gf.fig_mapa(modo, ir_g, Ks2, Ps_ir, "Strike K (MXN)", f"Precio del {tipo} (MXN/acción)",
